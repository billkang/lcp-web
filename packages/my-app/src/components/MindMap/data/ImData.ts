...
import type { Data, Mdata, IsMdata, GetTextSize } from '../types';
...
class ImData {
  data: Mdata;
  private getTextSize: GetTextSize;
  ...
  constructor(
    ...
    this.data = this.createMdataFromData(rawData, '0');
  ...
  private createMdataFromData(rawData: Data, id: string, parent: IsMdata = null): Mdata {
    ...
    const { width, height } = this.getTextSize(name);
  ...
  private renew(...plugins: Processer[]): void {
    ...
    this.data = this.separateTree(this.data);
   ...
   */
  private separateTree(data: Mdata): Mdata {
  ...
  rename(id: string, name: string): IsMdata {
    // 修改名称
    if (id.length > 0) {
      ...
      if (d && d.name !== name) {
        ...
        d.rawData.name = name;
        //
        const size = this.getTextSize(d.name);
        d.width = size.width;
   ...
   * @param delId - 被移动节点b
   */
  moveChild(targetId: string, delId: string): IsMdata {
    if (targetId === delId) {
      return null;
    }
    ...
    if (delIndex && target && del) {
      ...
      if (target.collapse) {
        target._children.push(del);
      } else {
        target.children.push(del);
      }
      target.rawData.children ? target.rawData.children.push(del.rawData) : (target.rawData.children = [del.rawData]);
  ...
  // 同层调换顺序
  moveSibling(id: string, referenceId: string, after = 0): IsMdata {
  ...
  add(id: string, variable: string | Data): IsMdata {
    ...
    if (p) {
      ...
      if (typeof variable === 'string') {
        const name = variable;
        const size = this.getTextSize(name);
  ...
  expand(id: string): IsMdata {
    return this.toggle(id, false, [renewColor, renewId]);
  }
   ...
   */
  toggle(id: string, collapse: boolean, plugins: Processer[] = []): IsMdata {
    const d = this.find(id);
    if (d) {
      d.collapse = collapse;
